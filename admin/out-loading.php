...
//获取总数量
$sql = "SELECT * FROM out_ WHERE staff_id='0' AND out_good='1'";
...
$sql = "SELECT * FROM out_ WHERE staff_id='0' AND out_good='1' LIMIT $startFrom, $onePageNum ";
$result = mysqli_query($link, $sql); //获取数据集
...
<script type="text/javascript">
    ...
    //定义一个JS的提示函数
    function outLoadingGo(outID) {

        var staffID = prompt("请安排配送员", "1");

        if (staffID != null  && staffID.valueOf() > 0) {
            //如果单击"确定"按钮，跳转到补货页面
            location.href = "out-loading-ing.php?id=" + outID + "&staffID=" + staffID;
        } else {
            alert("取消配送");
        }
    }
    ...
    //定义一个JS的提示函数
    function outLoadingSumGo(id) {

        var sum = prompt("请输入新金额", "");

        if (sum != null  && sum.valueOf() > 0) {
            //如果单击"确定"按钮，跳转到补货页面
            location.href = "out-loading-EditSum.php?id=" + id + "&sum=" + sum;
        } else {
            alert("取消修改");
        }
    }
...
<div class="con">
    ...
    <div class="right">
        ...
        <center>
            ...
            //根据搜索结果显示数据
            if ($totalRecords == 0) {
                echo "<br><br><br> <font color=red> 没有等待配送的订单呢 >_< </font> <br><br><br>";
                die();
            }
            ?>
            ...
            <table width="1050" border="1" align="center" rules="all" cellpadding="5">
                ...
                foreach ($arrs as $arr) { ?>
                    ...
                    <tr align="center">
                        ...
                        <td>
                            <?php echo $arr['out_sum'] ?>
                            <a href="#" onClick="outLoadingSumGo(<?php echo $arr['out_id'] ?>)">修改</a>
                        </td>
                        <td>
                            <?php
                            if (trim($arr['staff_id']) == 0) {
                        ...
                        <td>
                            <a href="#" onClick="outLoadingGo(<?php echo $arr['out_id'] ?>)">配送</a> |
                            <a href="#" onClick="outCloseGO(<?php echo $arr['out_id'] . "," . $arr['out_num'] . "," . $arr['water_id'] ?>)">关闭</a>
                        </td>
            ...
            //第一页
            echo "<a href='out-loading.php?page=1'>" . '第一页' . "</a> ";

            //循环输出中间的页
            for ($i = 1; $i <= $totalPages; $i++) {
                echo "<a href='out-loading.php?page=" . $i . "'> &nbsp;" . $i . "&nbsp; </a> ";
            };

            //最后一页
            echo "<a href='out-loading.php?page=$totalPages'>" . '最后一页' . "</a> ";
