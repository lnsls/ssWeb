...
//获取总数量
$sql = "SELECT * FROM water WHERE water_note='在售'";
...
$sql = "SELECT * FROM water WHERE water_note='在售' LIMIT $startFrom, $onePageNum ";
$result = mysqli_query($link, $sql); //获取数据集
...
<script type="text/javascript">
    ...
    //定义一个JS的提示函数
    function onGo(waterid) {

        if (window.confirm("确定要下架吗？")) {
            //如果单击"确定"按钮，跳转到下架页面
            location.href = "water-down-go.php?id=" + waterid;
        }
...
<div class="con">
    ...
    <div class="right">
        ...
        <center>
            ...
            //根据搜索结果显示数据
            if ($totalRecords == 0) {
                echo "<br><br><br> <font color=red> 没有可以下架的商品呢 >_< </font> <br><br><br>";
                die();
            }
            ?>
            ...
            <table width="1050" border="1" align="center" rules="all" cellpadding="5">
                ...
                foreach ($arrs as $arr) { ?>
                    ...
                    <tr align="center">
                        ...
                        <td>
                            <a href="#" onClick="onGo(<?php echo $arr['water_id'] ?>)">下架</a>
                        </td>
            ...
            //第一页
            echo "<a href='water-down.php?page=1'>" . '第一页' . "</a> ";

            //循环输出中间的页
            for ($i = 1; $i <= $totalPages; $i++) {
                echo "<a href='water-down.php?page=" . $i . "'> &nbsp;" . $i . "&nbsp; </a> ";
            };

            //最后一页
            echo "<a href='water-down.php?page=$totalPages'>" . '最后一页' . "</a> ";
