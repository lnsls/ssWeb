...
//获取总数量
$sql = "SELECT * FROM out_ WHERE staff_id!='0' AND out_ok='1' AND out_good='1'";
...
$sql = "SELECT * FROM out_ WHERE staff_id!='0' AND out_ok='1' AND out_good='1' LIMIT $startFrom, $onePageNum ";
$result = mysqli_query($link, $sql); //获取数据集
...
<script type="text/javascript">
    ...
    //定义一个JS的提示函数
    function outMoneyGo(id) {

        if (window.confirm("确定收款？")) {

            //如果单击"确定"按钮，跳转到上架页面
            location.href = "out-ok-money.php?id=" + id;
        }
    }
...
<div class="con">
    ...
    <div class="right">
        ...
        <center>
            ...
            //根据搜索结果显示数据
            if ($totalRecords == 0) {
                echo "<br><br><br> <font color=red> 没有送达的订单呢 >_< </font> <br><br><br>";
                die();
            }
            ?>
            ...
            <table width="1050" border="1" align="center" rules="all" cellpadding="5">
                ...
                foreach ($arrs as $arr) { ?>
                    ...
                    <tr align="center">
                        ...
                        <td>
                            <a href="#" onClick="outMoneyGo(<?php echo $arr['out_id'] ?>)">收款</a> |
            ...
            //第一页
            echo "<a href='out-ok.php?page=1'>" . '第一页' . "</a> ";

            //循环输出中间的页
            for ($i = 1; $i <= $totalPages; $i++) {
                echo "<a href='out-ok.php?page=" . $i . "'> &nbsp;" . $i . "&nbsp; </a> ";
            };

            //最后一页
            echo "<a href='out-ok.php?page=$totalPages'>" . '最后一页' . "</a> ";
