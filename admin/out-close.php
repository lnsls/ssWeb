...
//获取总数量
$sql = "SELECT * FROM out_ WHERE out_good='0' ORDER BY out_id DESC";
...
<script type="text/javascript">
    ...
    function outIngGo(id) {
        ...
        if (window.confirm("确定删除？")) {
            ...
            //如果单击"确定"按钮，跳转到上架页面
            location.href = "out-close-del.php?id=" + id;
        }
    }

    //定义一个JS的提示函数
    function outToGoodGO(id, num, water) {

        if (window.confirm("确定恢复？")) {

            //如果单击"确定"按钮，跳转到上架页面
            location.href = "out-togood.php?id=" + id + "&num=" + num + "&water=" + water;
        }
    }
...
<div class="con">
    ...
    <div class="right">
        ...
        <center>
            ...
            //根据搜索结果显示数据
            if ($totalRecords == 0) {
                echo "<br><br><br> <font color=red> 没有失效的订单呢 >_< </font> <br><br><br>";
                die();
            }
            ?>
            ...
            <table width="1050" border="1" align="center" rules="all" cellpadding="5">
                ...
                foreach ($arrs as $arr) { ?>
                    ...
                    <tr align="center">
                        ...
                        <td>
                            <a href="#" onClick="outIngGo(<?php echo $arr['out_id'] ?>)">删除</a> |
                            <a href="#"
                               onClick="outToGoodGO(<?php echo $arr['out_id'] . "," . $arr['out_num'] . "," . $arr['water_id'] ?>)">恢复</a>
                        </td>
            ...
            //第一页
            echo "<a href='out-close.php?page=1'>" . '第一页' . "</a> ";

            //循环输出中间的页
            for ($i = 1; $i <= $totalPages; $i++) {
                echo "<a href='out-close.php?page=" . $i . "'> &nbsp;" . $i . "&nbsp; </a> ";
            };

            //最后一页
            echo "<a href='out-close.php?page=$totalPages'>" . '最后一页' . "</a> ";
