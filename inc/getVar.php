...
function getWaterSumPayForCar($id, $num, $link)
{

    //1.先查购物车是什么水
    //执行查询的SQL语句
    $sql = "SELECT water_id FROM car WHERE car_id = $id";
    $result = mysqli_query($link, $sql);
...
function getWaterNum($id, $link)
{
    //执行查询的SQL语句
    $sql = "SELECT water_num FROM water WHERE water_id = $id";
    $result = mysqli_query($link, $sql);

    //获取所有行数据
    $arr = mysqli_fetch_assoc($result);

    return $arr['water_num'];
}
